...
func CopyDockerImage(client *regclient.RegClient, fromImageName string, toImageName string) error {
	...
	if errFrom != nil {
		LogError("Could not parse the from image name '%s'. Please ensure that the image name is correct.\n", fromImageName)
		return errFrom
	}

	rTo, errTo := ref.New(toImageName)
	if errTo != nil {
		LogError("Could not parse the to image name '%s'. Please ensure that the image name is correct.\n", toImageName)
		return errTo
	}
	...
	if err != nil {
		LogError("An error occurred when copying the image from '%s' to '%s'\n", fromImageName, toImageName)
		return err
	}
	return nil
}
